...
add_filter( 'grfwp_print_reviews_output', 'matt_custom_review_output', 10, 2 );
 function matt_custom_review_output( $output, $args ) {
	...
	if ( $reviews->have_posts() ) :
		...
		while( $reviews->have_posts() ) :
			...
			$post_meta['img'] = get_the_post_thumbnail( 
					get_the_ID(), 
					apply_filters( 'grfwp_the_post_thumbnail_size', 'post-footer-thumbnail' ), 
					array( 'class' => 'media-object circle') 
					) ;
			...
			<div <?php echo $grfwp_controller->format_classes( $classes ); ?> itemscope itemtype="http://schema.org/Review">
				...
				<?php else : ?>
						...
						<a class="pull-left" href="<?php echo esc_attr( $post_meta['review_url'] ); ?>">
							<?php if ( !empty( $post_meta['img'] ) ) : ?>
									<?php echo $post_meta['img']; ?>
							<?php else : ?>
									<img class="media-object circle" alt="No Author Image"  src="<?php bloginfo('template_url'); ?>/images/nopicture.jpg"/>
				...
				<div class="media-body">
					...
					<h4 class="media-heading review-name"><?php echo the_title(); ?></h4>
					...
					<blockquote class="italic review-body" itemprop="reviewBody">
...
function grfwp_print_reviews( $args = array() ) {
	...
	$output = apply_filters( 'grfwp_print_reviews_output', true, $args );
	if ( $output !== false ) {
		return $output;
	}
